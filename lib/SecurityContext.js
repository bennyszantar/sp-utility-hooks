...
const getAdGroupMembership = (group) => {
  const spCtx = spscript.createContext(ctx);
  return spCtx.get(`/web/sitegroups/getbyname('${group}')/CanCurrentUserViewMembership`)
}
...
const checkIfGroupIsAllowed = async (userGroups, authGroups) => 
  userGroups.some(group => {
    if (authGroups.indexOf(group) === 0) return true;
    
    let adMembership = await getAdGroupMembership(group)
    if(adMembership.d.CanCurrentUserViewMembership) return true; 
  })
